        ...
        public IActionResult Index() {
            try {
                using (var channel = GrpcChannel.ForAddress("http://127.0.0.1:5491")) {
                    ...
                    var isDocumentOpen = client.IsDocumentOpen(new Empty());
                    if (isDocumentOpen != null) {
                        TempData["status"] = isDocumentOpen.Value
                            ? "Все готово к работе" : "Не открыт документ";
                    }
                }
            } catch {
        ...
        public IActionResult GetObjects() {
            using (var channel = GrpcChannel.ForAddress("http://127.0.0.1:5491")) {
                ...
                try {
                    ...
                    var objects = client.GetObjects(request);
                    if (objects != null) {
